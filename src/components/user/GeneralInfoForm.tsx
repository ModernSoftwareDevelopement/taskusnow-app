...
const validateSchema = Yup.object().shape({
  ...
  address: Yup.string().test('len', 'Address is too long', (val) => {
    if (val === undefined) return true;
    return val.length == 0 || val.length <= 2000;
  }),
  address_2: Yup.string().test('len', 'Address is too long', (val) => {
    if (val === undefined) return true;
    return val.length == 0 || val.length <= 2000;
  }),
});
                ...
                <VStack spacing={3}>
                  <FormControl>
                    ...
                    {errors.fullName && touched.fullName && (
                      <Box color="red">{errors.fullName}</Box>
                    )}
                  </FormControl>
                </VStack>
                ...
                <VStack spacing={3}>
                  <FormControl>
                    ...
                    <Field
                      ...
                      type="email"
                    />
                    {errors.email_2 && touched.email_2 && (
                      <Box color="red">{errors.email_2}</Box>
                    )}
                  </FormControl>
                ...
                <VStack spacing={3}>
                  ...
                  <FormControl>
                    ...
                    {errors.address_2 && touched.address_2 && (
                      <Box color="red">{errors.address_2}</Box>
                    )}
                  </FormControl>
                </VStack>
